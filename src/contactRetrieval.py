...
The response should strictly adhere to the JSON format: {"results": [{"contacts": {"email": " Email of the vendor","phone": "Phone number of the vendor","address": "Address of the vendor"},"name": "Name of the vendor helping the goal","info": "Describe the service provider and their service in 30-50 words(Optional)","source": "Source website link of the information from metadata website","provider": ["Source from 'Google', 'Bing' or both" or also 'Google Maps']}, {...}]}.
Use an empty string "" if any is absent or any data is not available. Avoid providing incorrect contact details. Give phone numbers(in E.164 Format) and emails in usable and correct format (no helper words). If any contact information is unavailable, just omit the service provider. Give only one email and one phone number for each vendor/person.
Do not give dummy or example data, if the context data is not enough the omit the service provider. Ensure extracted Vendor and contact are relevant to solution and capable of solving the goal. Make sure the phone number is in E.164 Format, based on location.
