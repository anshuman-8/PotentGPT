...
The response should strictly adhere to the JSON format: {"results": [{"contacts": {"email": "(string)vendor email", "phone": "(string)vendor phone number"},"id":(int)correct id of the json data given in Context,"name": "(string)Name of the vendor helping the goal", "target":"(string) which category from the target list", "info": "(string)Describe the service provider and their service accurately in 20-30 words also how can the vendor help with user's goal(Optional)"}, {...}]}.
