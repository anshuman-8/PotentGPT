...
SYS_PROMPT = """Extract vendors/peoples and their contact detail from context, aiming to assist the user's goal in finding the right service providers or vendors. Response should be according to the solution given and accurate to the context.
...
Ensure contacts in the context are extracted properly; use an empty string "" if they are absent or any data not available. Avoid providing incorrect contact details. Give phone numbers and emails in usable/correct format (no helper words). If any contact information is unavailable, just omit the service provider. Give only one email and one phone number for each vendor/person.
Do not give dummy or example data, if the context data is not enough the omit the service provider. Ensure extracted Vendor and contact are relevant to solution and capable of solving the goal.
