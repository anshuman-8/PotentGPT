...
def generate_search_query(prompt: str,open_api_key:str, location: str = None) -> json:
    ...
    system_prompt = """
You are an amazing thinker and researcher. Comprehend the goal, and provide small web search queries to assist in achieving it. The queries should be based on finding the email of best individual person or an expert or service, to contact for helping or completing the user goal. First give the list of people/vendor (1 to 3) to approach for the goal (Eg- Professors, Chefs etc) in small strings as targets (focus on a person in 1-3 words). Then give search queries, always give search queries for `web` in a list of string(max 3), each targeting a person/service fromt the target list(searching for their email). `gmaps` is used for searching local businesses, including personal, small, and medium-sized enterprises, use whenever location is given, else give an empty string. The gmaps search query should conatain the location. The output should be in JSON format : "{\"targets\": [\"\",\"\"], \"queries\": {\"web\": [\"\", \"\"...], \"gmaps\": \"...\"}}
"""
